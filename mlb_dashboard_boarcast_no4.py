...
# ==========================================================
# 🌐 語言選擇 / Language Toggle
# ==========================================================
# 提供「繁體中文」與「English」雙語切換。UI 上所有文字一律透過 t(zh, en)
# 依目前 st.session_state["lang"] 取得對應語言的字串；PIL / Plotly 等非
# Streamlit widget 的繪圖函式則另外接收 lang 參數（因為它們在 script rerun
# 之外仍需要知道目前語言，不能直接讀取 st.session_state）。
LANGUAGES = {"zh": "繁體中文", "en": "English"}


def get_lang() -> str:
    """回傳目前選定的語言代碼（'zh' 或 'en'），預設為 'zh'。"""
    return st.session_state.get("lang", "zh")


def t(zh: str, en: str) -> str:
    """雙語文字輔助函式：依目前語言設定回傳繁體中文或英文文字。"""
...
# [Bilingual] 熱區指標改用「不隨語言變動」的內部鍵值（HOT_ZONE_METRIC_KEYS），
# 顯示文字則透過 hot_zone_metric_label() 依目前語言即時查表，
# 避免翻譯後字串比對（render_hot_zone 內的邏輯判斷）跟著失效。
HOT_ZONE_METRIC_KEYS = ["AVG", "SLG", "ISO", "HR", "3B", "2B", "1B", "AVG_EV"]

_HOT_ZONE_LABELS_ZH = {
    "AVG": "打擊率 (AVG)", "SLG": "長打率 (SLG)", "ISO": "純長打率 (ISO)",
    "HR": "全壘打 (HR)", "3B": "三壘安打 (3B)", "2B": "二壘安打 (2B)",
    "1B": "一壘安打 (1B)", "AVG_EV": "平均擊球初速 (Avg EV)",
}
_HOT_ZONE_LABELS_EN = {
    "AVG": "Batting Avg (AVG)", "SLG": "Slugging % (SLG)", "ISO": "Isolated Power (ISO)",
    "HR": "Home Runs (HR)", "3B": "Triples (3B)", "2B": "Doubles (2B)",
    "1B": "Singles (1B)", "AVG_EV": "Avg Exit Velo (Avg EV)",
}


def hot_zone_metric_label(key: str) -> str:
    return (_HOT_ZONE_LABELS_EN if get_lang() == "en" else _HOT_ZONE_LABELS_ZH).get(key, key)
...
# 球種代碼 -> 顯示名稱（供 #2 球種過濾器使用；依語言分別提供中/英文對照表）
PITCH_TYPE_NAMES_ZH = {
...
PITCH_TYPE_NAMES_EN = {
    "FF": "4-Seam Fastball FF", "SI": "Sinker SI", "FC": "Cutter FC",
    "SL": "Slider SL", "ST": "Sweeper ST", "SV": "Slurve SV",
    "CU": "Curveball CU", "KC": "Knuckle Curve KC", "CS": "Slow Curve CS",
    "CH": "Changeup CH", "FS": "Splitter FS", "FO": "Forkball FO",
    "SC": "Screwball SC", "KN": "Knuckleball KN", "EP": "Eephus EP",
    "PO": "Pitchout PO", "FA": "Fastball (Unclassified) FA",
}


def pitch_type_label(code: str) -> str:
    d = PITCH_TYPE_NAMES_EN if get_lang() == "en" else PITCH_TYPE_NAMES_ZH
    return d.get(code, code)


# [Bilingual] 左右投拆分改用內部鍵值，顯示文字依語言查表（見 platoon_label()）
PLATOON_KEYS = ["ALL", "LHP", "RHP"]
_PLATOON_LABELS_ZH = {"ALL": "全部 (All)", "LHP": "面對左投 vs LHP", "RHP": "面對右投 vs RHP"}
_PLATOON_LABELS_EN = {"ALL": "All", "LHP": "vs LHP", "RHP": "vs RHP"}


def platoon_label(key: str) -> str:
    return (_PLATOON_LABELS_EN if get_lang() == "en" else _PLATOON_LABELS_ZH).get(key, key)
...
def render_hot_zone(pa_df: pd.DataFrame, metric_choice: str) -> go.Figure:
    ...
    label_text = hot_zone_metric_label(metric_choice)
    fig = px.imshow(
        grid_vals,
        color_continuous_scale="RdBu_r",
        labels=dict(color=label_text),
        x=[t("內角 In", "Inside"), t("中間 Mid", "Mid"), t("外角 Out", "Outside")],
        y=[t("高角 High", "High"), t("中間 Mid", "Mid"), t("低角 Low", "Low")],
        aspect="equal",
    )
    fig.update_traces(
        text=grid_txts,
        ...
        hovertemplate="%{y} / %{x}<br>" + t("數值", "Value") + ": %{z}<extra></extra>",
    )
    fig.update_layout(
        title=t("好球帶熱區分析 - ", "Hot Zone Analysis - ") + label_text,
        width=440,
        height=440,
...
def render_spray_chart(
    ...
    if spray_df.empty:
        fig = go.Figure()
        fig.update_layout(
            title=t("擊球落點分佈（此區間無安打座標資料）", "Spray Chart (No hit coordinate data in this range)"),
    ...
    if spray_df.empty:
        fig = go.Figure()
        fig.update_layout(
            title=t("擊球落點分佈（目前篩選條件下無符合資料）", "Spray Chart (No data matches current filters)"),
            width=700, height=700,
        )
        return fig
    ...
    EVENT_LABEL_MAP = {
        "single": t("一壘安打", "Single"),
        "double": t("二壘安打", "Double"),
        "triple": t("三壘安打", "Triple"),
        "home_run": t("全壘打", "Home Run"),
    }
    spray_df["result"] = spray_df["events"].map(EVENT_LABEL_MAP)

    color_map = {
        EVENT_LABEL_MAP["single"]: "#1f77b4",     # 藍色 — single
        EVENT_LABEL_MAP["double"]: "#2ca02c",     # 綠色 — double
        EVENT_LABEL_MAP["triple"]: "#ff7f0e",     # 橘色 — triple
        EVENT_LABEL_MAP["home_run"]: "#d62728",   # 紅色 — home_run
    }
    result_order = [EVENT_LABEL_MAP["home_run"], EVENT_LABEL_MAP["triple"],
                     EVENT_LABEL_MAP["double"], EVENT_LABEL_MAP["single"]]
    ...
    for label in result_order:
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=(
                f"<b>{label}</b><br>"
                f"{t('擊球初速', 'Exit Velo')}：%{{customdata[0]}}<br>"
                f"{t('仰角', 'Launch Angle')}：%{{customdata[1]}}<br>"
                f"{t('飛球距離', 'Distance')}：%{{customdata[2]}}"
                "<extra></extra>"
            ),
        ))
        n_hit_traces += 1

    fig.update_layout(title=t("擊球落點分佈圖（可拖曳滑鼠框選 Box Select 過濾）",
                               "Spray Chart (Drag to Box Select and filter)"))
    ...
    stadium_label = home_team if home_team else t("通用", "Generic")
    ...
    fig.add_trace(go.Scatter(
        ...
        name=t(f"全壘打牆（{stadium_label} 主場）", f"HR Wall ({stadium_label} Home)"), hoverinfo="skip",
    ))
    ...
    fig.update_layout(
        ...
        legend_title_text=t("安打類型", "Hit Type"),  # [2026 Rev 8] 圖例標題另翻譯，避免預設顯示英文欄位名稱 "result"
...
def render_rolling_ops_chart(pa_df_full: pd.DataFrame, player_name: str, window: int = 15):
    ...
    fig.add_trace(go.Scatter(
        ...
        name=t(f"近 {window} 場滾動 OPS", f"Rolling OPS (last {window} games)"),
        ...
        # 兩者互不干擾。
        hovertemplate=t(f"第 {{x}} 場<br>近{window}場滾動 OPS: {{y:.3f}}<extra></extra>",
                         f"Game {{x}}<br>Rolling OPS (last {window}): {{y:.3f}}<extra></extra>"),
    ))
    fig.add_hline(
        y=season_avg_ops, line=dict(color="#FFD54F", dash="dash", width=1.5),
        annotation_text=t(f"賽季均值 {season_avg_ops:.3f}", f"Season Avg {season_avg_ops:.3f}"),
        annotation_position="top left",
    )
    fig.update_layout(
        title=t(f"🔥 {player_name} 全賽季手感起伏圖（近 {window} 場動態滾動 OPS）",
                f"🔥 {player_name} Season Momentum (Rolling OPS, last {window} games)"),
        xaxis_title=t("場次 (Game Number)", "Game Number"),
...
def render_pk_rolling_ops_chart(pa_df_a: pd.DataFrame, name_a: str,
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines", line=dict(color="#00E676", width=3),
        name=f"🔵 {name_a}",
        hovertemplate=t(f"{name_a}｜第 {{x}} 場<br>近{window}場滾動 OPS: {{y:.3f}}<extra></extra>",
                        f"{name_a} | Game {{x}}<br>Rolling OPS (last {window}): {{y:.3f}}<extra></extra>"),
    ))
    fig.add_trace(go.Scatter(
        x=roll_b["game_num"], y=roll_b["rolling_ops"],
        mode="lines", line=dict(color="#FF5252", width=3),
        name=f"🔴 {name_b}",
        hovertemplate=t(f"{name_b}｜第 {{x}} 場<br>近{window}場滾動 OPS: {{y:.3f}}<extra></extra>",
                        f"{name_b} | Game {{x}}<br>Rolling OPS (last {window}): {{y:.3f}}<extra></extra>"),
    ))
    fig.update_layout(
        title=t(f"🥊 {name_a} vs {name_b}　近 {window} 場滾動 OPS 對比",
                f"🥊 {name_a} vs {name_b}　Rolling OPS (last {window} games)"),
        xaxis_title=t("場次 (Game Number)", "Game Number"),
        yaxis_title="Rolling OPS",
        width=980, height=380,
        margin=dict(l=60, r=40, t=60, b=50),
...
def _fig_to_pil(fig: go.Figure, w: int, h: int) -> Image.Image:
    try:
        ...
        d.text((20, h // 2), t("圖表匯出需安裝 kaleido 套件", "Chart export requires the kaleido package"), fill="white")
        return placeholder
...
def generate_scouting_card(
    ...
    mode_text = (
        t("🚀 162 場全賽季模擬預測", "🚀 162-Game Season Projection") if is_scaled
        else t(f"選定區間：第 {game_range[0]}~{game_range[1]} 場", f"Selected range: Game {game_range[0]}~{game_range[1]}")
    )
    draw.text((60, 155), t(f"{year} 賽季　|　{mode_text}", f"{year} Season　|　{mode_text}"), fill="#BBBBBB", font=body_font)
    ...
    ev_txt = (
        ...
        else t("Statcast 物理數據不足", "Insufficient Statcast physical data")
    )
...
def apply_platoon_pitch_filter(df: pd.DataFrame, platoon_choice: str, selected_pitch_types: list) -> pd.DataFrame:
    ...
    out = df
    if platoon_choice == "LHP" and "p_throws" in out.columns:
        out = out[out["p_throws"] == "L"]
    elif platoon_choice == "RHP" and "p_throws" in out.columns:
...
def render_platoon_pitch_filters(pa_df: pd.DataFrame, key_prefix: str):
    ...
    with colf1:
        platoon_choice = st.selectbox(
            t("⚔️ 左右投拆分 (Platoon Splits)", "⚔️ Platoon Splits"),
            PLATOON_KEYS,
            format_func=platoon_label,
    ...
    with colf2:
        ...
        pitch_labels = [pitch_type_label(pt) for pt in available_pitch_types]
        label_to_code = dict(zip(pitch_labels, available_pitch_types))
        selected_labels = st.multiselect(
            t("🎯 球種過濾器 (Pitch Type Filter)", "🎯 Pitch Type Filter"),
            pitch_labels,
...
def generate_ig_story_card(
    ...
    mode_text = (
        t("🚀 162 場全賽季模擬", "🚀 162-Game Simulation") if is_scaled
        else t(f"第 {game_range[0]}~{game_range[1]} 場", f"Game {game_range[0]}~{game_range[1]}")
    )
    tag_text = t(f"{year} 賽季　|　{mode_text}", f"{year} Season　|　{mode_text}")
...
def generate_scouting_card_cached(player_name, year, stats, is_scaled, game_range,
    ...
    key = ("scouting", get_lang(), player_name, year, tuple(game_range), is_scaled, _stats_fingerprint(stats))
...
def generate_ig_story_card_cached(player_name, year, stats, is_scaled, game_range,
    ...
    key = ("ig", get_lang(), player_name, year, tuple(game_range), is_scaled, _stats_fingerprint(stats))
...
def generate_pk_card_cached(buf_a: io.BytesIO, buf_b: io.BytesIO, name_a: str, name_b: str,
    ...
    key = ("pk", get_lang(), name_a, name_b, fingerprint_a, fingerprint_b)
...
def load_player(fn: str, ln: str, year: int):
    ...
    try:
        ...
        st.error(t(f"抓取 {fn} {ln} 資料時發生錯誤：{e}", f"Error fetching data for {fn} {ln}: {e}"))
        return None, None, None
    ...
    if pa_df is None:
        if player_id is None:
            st.error(t(f"找不到球員 {fn} {ln}，請確認姓名拼寫是否正確。",
                       f"Could not find player {fn} {ln}. Please check the spelling."))
        else:
            st.info(t(f"ℹ️ {fn} {ln} 於 {year} 賽季無例行賽擊球數據，暫無法顯示分析面板。",
                      f"ℹ️ No regular-season batting data for {fn} {ln} in {year}; unable to show the panel."))
        return None, None, None
...
def render_player_panel(pa_df: pd.DataFrame, player_name: str, player_id: int,
    ...
    with col_info:
        ...
        st.caption(t(f"總打席：{len(pa_df)}　|　球隊：{team}　|　打擊習慣：{bats}",
                     f"Total PA: {len(pa_df)}　|　Team: {team}　|　Bats: {bats}"))
    ...
    st.caption(t("⏱️ 近期手感快捷鍵", "⏱️ Recent Form Shortcuts"))
    qb1, qb2, qb3, qb_spacer = st.columns([1, 1, 1, 3])
    if qb1.button(t("🔥 近 7 場", "🔥 Last 7"), key=f"{key_prefix}_quick7"):
        st.session_state[range_key] = (max(1, max_games - 7 + 1), max_games)
        st.rerun()
    if qb2.button(t("⚡ 近 15 場", "⚡ Last 15"), key=f"{key_prefix}_quick15"):
        st.session_state[range_key] = (max(1, max_games - 15 + 1), max_games)
        st.rerun()
    if qb3.button(t("📊 近 30 場", "📊 Last 30"), key=f"{key_prefix}_quick30"):
        st.session_state[range_key] = (max(1, max_games - 30 + 1), max_games)
        st.rerun()

    game_range = st.slider(
        t("選定場次區間", "Selected Game Range"), min_value=1, max_value=slider_max,
    ...
    if extrapolation_allowed:
        scale_to_162 = st.checkbox(t("🚀 外推至 162 場模擬", "🚀 Project to 162 Games"), value=False, key=f"{key_prefix}_scale")
    else:
        st.checkbox(
            t("🚀 外推至 162 場模擬", "🚀 Project to 162 Games"), value=False, key=f"{key_prefix}_scale",
            disabled=True,
            help=t(
                f"選定區間僅 {games_in_sample_preview} 場比賽，未達最低 {MIN_GAMES_FOR_EXTRAPOLATION} 場門檻，暫不開放外推模擬。",
                f"Selected range has only {games_in_sample_preview} games, below the minimum of "
                f"{MIN_GAMES_FOR_EXTRAPOLATION} games required for projection.",
            ),
        )
        scale_to_162 = False
        st.caption(t(
            f"ℹ️ 選定區間僅 {games_in_sample_preview} 場比賽（未達 {MIN_GAMES_FOR_EXTRAPOLATION} 場門檻），"
            f"外推至 162 場功能暫時停用，避免賽季初期小樣本產生失真預測。",
            f"ℹ️ Selected range has only {games_in_sample_preview} games (below the "
            f"{MIN_GAMES_FOR_EXTRAPOLATION}-game threshold). 162-game projection is disabled to avoid "
            f"distorted predictions from small early-season samples.",
        ))
    ...
    if scale_to_162 and (stats.get("Extrap_Capped") or stats.get("Extrap_LowSample")):
        if stats.get("Extrap_Capped"):
            st.warning(t(
                ...
                f"⚠️ Selected range has only {games_in_sample} games. The projection factor has been "
                f"capped at {stats['Extrap_Factor']:.1f}x to avoid unrealistic extreme predictions; "
                f"treat this result as a rough reference only.",
            ))
        elif stats.get("Extrap_LowSample"):
            st.warning(t(
                f"⚠️ 選定區間僅 {games_in_sample} 場比賽，樣本數過少，外推結果僅供參考。",
                f"⚠️ Selected range has only {games_in_sample} games — too small a sample. "
                f"Treat this projection as a rough reference only.",
            ))
    ...
    st.caption(t(
        ...
        f"📈 Expected Stats　xBA {xba_txt}　|　xSLG {xslg_txt}　|　xwOBA {xwoba_txt}　|　"
        f"Barrel% {barrel_txt}　|　Hard Hit% {hardhit_txt}",
    ))
    ...
    st.caption(t(
        f"🎯 擊球型態分佈　滾地球 GB% {gb_txt}　|　平飛球 LD% {ld_txt}　|　"
        f"飛球 FB% {fb_txt}　|　內野高飛 PU% {pu_txt}",
        f"🎯 Batted Ball Type　Ground Ball GB% {gb_txt}　|　Line Drive LD% {ld_txt}　|　"
        f"Fly Ball FB% {fb_txt}　|　Pop Up PU% {pu_txt}",
    ))

    metric_choice = st.radio(
        t("熱區指標", "Hot Zone Metric"), HOT_ZONE_METRIC_KEYS, horizontal=True,
        format_func=hot_zone_metric_label,
    ...
    # 避免對空欄位計算 min/max 出錯。
    st.caption(t("🎯 落點分佈圖篩選（僅影響下方落點圖，不影響上方統計數據）",
                 "🎯 Spray Chart Filters (affects only the chart below, not the stats above)"))
    ...
    with spray_ev_col:
        if has_ev:
            ...
            if ev_min >= ev_max:
                ev_max = ev_min + 1.0
            ev_range = st.slider(
                t("擊球初速 EV 範圍 (mph)", "Exit Velo Range (mph)"), min_value=ev_min, max_value=ev_max,
                value=(ev_min, ev_max), key=f"{key_prefix}_ev_range",
            )
        else:
            ev_range = None
            st.caption(t("ℹ️ 此區間無擊球初速資料，篩選停用。", "ℹ️ No exit velo data in this range; filter disabled."))
    ...
    with spray_la_col:
        if has_la:
            ...
            if la_min >= la_max:
                la_max = la_min + 1.0
            la_range = st.slider(
                t("發射角 LA 範圍 (°)", "Launch Angle Range (°)"), min_value=la_min, max_value=la_max,
                value=(la_min, la_max), key=f"{key_prefix}_la_range",
            )
        else:
            la_range = None
            st.caption(t("ℹ️ 此區間無發射角資料，篩選停用。", "ℹ️ No launch angle data in this range; filter disabled."))
...
def main():
    ...
    # ---------------- 🌐 語言選擇 / Language ----------------
    # 放在最頂端、st.title() 之前，確保語言選擇一改變，整頁（含標題）就立刻切換。
    st.session_state.setdefault("lang", "zh")
    lang_col, _spacer = st.columns([1, 5])
    with lang_col:
        ...
        st.session_state["lang"] = [k for k, v in LANGUAGES.items() if v == selected_lang_label][0]

    inject_cyberpunk_theme()
    st.title(t("⚾ MLB 擊球數據模擬與球員視覺化儀表板", "⚾ MLB Batting Simulator & Player Dashboard"))
    st.caption(t(
        f"🔵 電子發光霓虹藍＝一般數據　|　🌸 極光霓虹粉紅＝極端數據警示 "
        f"(Max EV ≥ {EXIT_VELO_ALERT_MPH:.0f}mph 或面對火球 ≥ {PITCH_VELO_ALERT_MPH:.0f}mph)",
        f"🔵 Neon Blue = normal stats　|　🌸 Neon Pink = extreme-stat alert "
        f"(Max EV ≥ {EXIT_VELO_ALERT_MPH:.0f}mph or facing a pitch ≥ {PITCH_VELO_ALERT_MPH:.0f}mph)",
    ))

    if not PYBASEBALL_AVAILABLE:
        st.error(t("⚠️ 尚未安裝 pybaseball，請先執行：pip install pybaseball",
                   "⚠️ pybaseball is not installed. Please run: pip install pybaseball"))
        st.stop()
    ...
    if not check_cjk_font_status():
        st.sidebar.warning(t(
            ...
            "⚠️ No CJK-capable font detected in this environment. Exported PNG card titles "
            "may show missing-glyph boxes for Chinese text.\n\n"
            "Suggestion: add NotoSansTC-Regular.ttf (and the Bold variant) to the project's "
            "assets/ folder, or add a packages.txt with a line `fonts-noto-cjk` and redeploy.",
        ))
    ...
    # ---------------- 側邊欄：球員搜尋 ----------------
    st.sidebar.header(t("🔍 球員搜尋", "🔍 Player Search"))
    # 注意：不再傳入 value=，改為純 key 綁定，避免 Streamlit 對
    # 「同時提供 value 與 session_state key」丟出警告，也才能讓
    # 快捷鍵點擊確實反映在輸入框上。
    first_name = st.sidebar.text_input(t("名 (First Name)", "First Name"), key="first_name")
    last_name = st.sidebar.text_input(t("姓 (Last Name)", "Last Name"), key="last_name")
    selected_year = st.sidebar.selectbox(t("選擇賽季年份", "Select Season Year"), get_selectable_years(), index=0)
    ...
    st.sidebar.markdown(t("**⭐ 熱門球員快捷鍵**", "**⭐ Popular Player Shortcuts**"))
    ...
    st.sidebar.button(t("🎲 隨機載入明星球員", "🎲 Load a Random Star Player"), on_click=_apply_random_player)

    load_clicked = st.sidebar.button(t("📥 載入球員數據", "📥 Load Player Data"), type="primary")
    ...
    st.sidebar.markdown("---")
    if st.sidebar.button(t("🧹 清除記憶體快取", "🧹 Clear Memory Cache")):
        st.session_state["_player_cache"] = {}
        st.session_state["_card_cache"] = {}
        st.sidebar.success(t("已釋放球員資料與圖卡快取記憶體！", "Player data and card caches have been cleared!"))
    ...
    if load_clicked or st.session_state.pop("pending_load", False):
        ...
        with st.spinner(t(f"正在抓取 {fn} {ln} 的 MLB Statcast 資料...", f"Fetching MLB Statcast data for {fn} {ln}...")):
    ...
    if "pa_df" not in st.session_state:
        st.info(t(
            "👈 請於左側輸入球員姓名並點擊「載入球員數據」開始使用（或試試 🎲 隨機載入明星球員）。",
            "👈 Enter a player's name on the left and click \"Load Player Data\" to get started "
            "(or try 🎲 Load a Random Star Player).",
        ))
        return

    # ---------------- 主分頁：單人分析 vs 雙人 PK 對比（功能 #3） ----------------
    tab_solo, tab_pk = st.tabs([
        t("📊 球員分析主板", "📊 Player Analysis"),
        t("🥊 雙球員 PK 對比模式", "🥊 Head-to-Head Comparison"),
    ])
    ...
    with tab_solo:
        ...
        st.subheader(t("🎛️ 微觀時間軸、左右投拆分與球種過濾控制器", "🎛️ Timeline, Platoon Split & Pitch Type Controls"))
        ...
        st.markdown(t("### 📊 戰績數據指標", "### 📊 Stat Line"))
        if scale_to_162:
            st.caption(t(f"⚠️ 以下數據為【第 {game_range[0]}~{game_range[1]} 場】外推至 162 場的模擬預測值",
                         f"⚠️ The stats below project [Game {game_range[0]}~{game_range[1]}] to a 162-game season"))
        else:
            st.caption(t(f"顯示【第 {game_range[0]}~{game_range[1]} 場】實際數據",
                         f"Showing actual data for [Game {game_range[0]}~{game_range[1]}]"))

        # [UI Fix: 模組二] 欄數網格重構：原本單排硬塞 6 欄導致欄寬不足
        # （率數據被截斷成 0....，數字與單位如 118.5 mph 異常跳行），
        # 改為每排 3 欄、分兩排顯示，讓每張卡片有足夠的橫向展延空間。
        m1, m2, m3 = st.columns(3)
        m1.metric(t("打擊率 AVG", "Batting Avg (AVG)"), f"{stats['AVG']:.3f}")
        m2.metric(t("上壘率 OBP", "On-Base % (OBP)"), f"{stats['OBP']:.3f}")
        m3.metric(t("長打率 SLG", "Slugging % (SLG)"), f"{stats['SLG']:.3f}")

        m4, m5, m6 = st.columns(3)
        m4.metric(t("攻擊指數 OPS", "OPS"), f"{stats['OPS']:.3f}")
        m5.metric(t("純長打率 ISO", "Isolated Power (ISO)"), f"{stats['ISO']:.3f}")
        ev_display = f"{stats['Avg_EV']:.1f} mph" if not np.isnan(stats['Avg_EV']) else "N/A"
        m6.metric(t("平均初速 Avg EV", "Avg Exit Velo (Avg EV)"), ev_display)

        n1, n2, n3 = st.columns(3)
        n1.metric(t("一壘安打 1B", "Singles (1B)"), stats["1B"])
        n2.metric(t("二壘安打 2B", "Doubles (2B)"), stats["2B"])
        n3.metric(t("三壘安打 3B", "Triples (3B)"), stats["3B"])

        n4, n5, n6 = st.columns(3)
        n4.metric(t("全壘打 HR", "Home Runs (HR)"), stats["HR"])
        n5.metric(t("總安打 H", "Hits (H)"), stats["H"])
        n6.metric(t("打點 RBI", "RBI"), stats["RBI"])

        p1, p2, p3 = st.columns(3)
        p1.metric(t("盜壘 SB", "Stolen Bases (SB)"), stats["SB"])
        p2.metric(t("盜壘失敗 CS", "Caught Stealing (CS)"), stats["CS"])
        p3.metric(t("保送 BB", "Walks (BB)"), stats["BB"])

        p4, p5, p6 = st.columns(3)
        p4.metric(t("三振 SO", "Strikeouts (SO)"), stats["SO"])
        ...
        render_cyber_stat_tile(p5, t("最高初速 MAX EV", "Max Exit Velo (MAX EV)"), max_ev_display, is_alert=max_ev_alert)
        la_display = f"{stats['Avg_LA']:.1f}°" if not np.isnan(stats['Avg_LA']) else "N/A"
        p6.metric(t("平均仰角 Avg LA", "Avg Launch Angle (Avg LA)"), la_display)
        ...
        if not np.isnan(max_pitch_velo):
            ...
            render_cyber_stat_tile(fb1, t("本區間面對最快球速", "Fastest Pitch Faced (this range)"), fb_display, is_alert=fb_alert)

        if not np.isnan(stats.get("Max_HR_Dist", np.nan)):
            # [Module 3] 距離屬於物理量，統一格式化至小數點後第 1 位
            st.caption(t(f"🏟️ 全壘打距離：最遠 {stats['Max_HR_Dist']:.1f} ft　|　平均 {stats['Avg_HR_Dist']:.1f} ft",
                         f"🏟️ HR Distance: Max {stats['Max_HR_Dist']:.1f} ft　|　Avg {stats['Avg_HR_Dist']:.1f} ft"))

        # [Enh 2 / Module 3] 進階期望數據 (xBA/xSLG/xwOBA/Barrel%/Hard Hit%) 指標列
        st.markdown(t("##### 📈 進階期望數據 (Statcast Expected Stats)", "##### 📈 Advanced Expected Stats (Statcast)"))
        q1, q2, q3, q4, q5 = st.columns(5)
        q1.metric(t("估計打擊率 xBA", "Expected AVG (xBA)"), f"{stats['xBA']:.3f}" if pd.notna(stats["xBA"]) else "N/A")
        q2.metric(t("估計長打率 xSLG", "Expected SLG (xSLG)"), f"{stats['xSLG']:.3f}" if pd.notna(stats["xSLG"]) else "N/A")
        q3.metric(t("估計加權上壘率 xwOBA", "Expected wOBA (xwOBA)"), f"{stats['xwOBA']:.3f}" if pd.notna(stats.get("xwOBA")) else "N/A")
        q4.metric(t("出色擊球率 Barrel%", "Barrel Rate (Barrel%)"), f"{stats['Barrel_Pct'] * 100:.1f}%" if pd.notna(stats["Barrel_Pct"]) else "N/A")
        q5.metric(t("強擊球率 Hard Hit%", "Hard Hit Rate (Hard Hit%)"), f"{stats['HardHit_Pct'] * 100:.1f}%" if pd.notna(stats["HardHit_Pct"]) else "N/A")

        # 擊球型態分佈 (GB% / LD% / FB% / PU%)
        st.markdown(t("##### 🎯 擊球型態分佈 (Batted Ball Type)", "##### 🎯 Batted Ball Type Distribution"))
        r1, r2, r3, r4 = st.columns(4)
        r1.metric(t("滾地球 GB%", "Ground Ball (GB%)"), f"{stats['GB_Pct'] * 100:.1f}%" if pd.notna(stats.get("GB_Pct")) else "N/A")
        r2.metric(t("平飛球 LD%", "Line Drive (LD%)"), f"{stats['LD_Pct'] * 100:.1f}%" if pd.notna(stats.get("LD_Pct")) else "N/A")
        r3.metric(t("飛球 FB%", "Fly Ball (FB%)"), f"{stats['FB_Pct'] * 100:.1f}%" if pd.notna(stats.get("FB_Pct")) else "N/A")
        r4.metric(t("內野高飛 PU%", "Pop Up (PU%)"), f"{stats['PU_Pct'] * 100:.1f}%" if pd.notna(stats.get("PU_Pct")) else "N/A")

        # 全賽季手感起伏圖 (Rolling OPS 走勢圖)：一律以整季 pa_df 計算，不受微觀區間篩選影響
        st.markdown("---")
        st.markdown(t("##### 📈 全賽季手感起伏圖 (15-Game Rolling OPS)", "##### 📈 Season Momentum (15-Game Rolling OPS)"))
        fig_rolling = render_rolling_ops_chart(pa_df, player_name, window=15)
        if fig_rolling is None:
            st.warning(t("⚠️ 該球員本賽季無足夠的例行賽擊球數據，暫無法繪製滾動 OPS 走勢圖。",
                         "⚠️ Not enough regular-season batting data this season to plot the rolling OPS chart."))
        else:
            st.plotly_chart(fig_rolling, use_container_width=True, key="solo_rolling_ops")

        st.markdown("---")
        st.subheader(t("📥 匯出個人專屬 Scouting Report 簡報卡", "📥 Export Personal Scouting Report Card"))
        if st.button(t("🖼️ 生成高清 Scouting Report 圖片 (PNG)", "🖼️ Generate HD Scouting Report Image (PNG)"), key="solo_export"):
            st.caption(t("⏳ 圖表合成中，約需 3~5 秒...（相同區間/外推設定重複產生時會直接讀取快取，秒開）",
                         "⏳ Compositing the chart, roughly 3–5 seconds... (Instant if the same range/settings were generated before, via cache)"))
            with st.spinner(t("正在合成報告卡片...", "Compositing report card...")):
            ...
            st.image(card_buf, caption=t("報告卡預覽", "Report Card Preview"), width=500)
            st.download_button(
                label=t("📩 點擊下載報告圖檔 (PNG)", "📩 Download Report Card (PNG)"), data=card_buf,
                file_name=f"{player_name.replace(' ', '_')}_scouting_report.png",
                mime="image/png", key="solo_download",
            )

        st.markdown(t("##### 📱 IG 限時動態專用圖卡（9:16 直式 1080x1920）", "##### 📱 IG Story Card (9:16 Portrait, 1080x1920)"))
        st.caption(t(
            "精簡排版、放大球員頭像、三圍數據與落點圖，並印上 boarcast 浮水印，免裁切一鍵發布限動。",
            "Compact layout with a larger headshot, key stats, and spray chart, plus the boarcast "
            "watermark — publish straight to your Story with no cropping needed.",
        ))
        if st.button(t("📱 生成 IG 限動直式圖卡 (PNG)", "📱 Generate IG Story Card (PNG)"), key="solo_export_ig"):
            st.caption(t("⏳ 圖表合成中，約需 3~5 秒...", "⏳ Compositing the card, roughly 3–5 seconds..."))
            with st.spinner(t("正在合成 IG 限動圖卡...", "Compositing IG Story card...")):
                ig_buf = generate_ig_story_card_cached(
                    player_name=player_name, year=year, stats=stats,
                    is_scaled=scale_to_162, game_range=game_range,
                    headshot_url=headshot_url, spray_fig=fig_spray,
                )
            st.image(ig_buf, caption=t("IG 限動圖卡預覽（9:16）", "IG Story Card Preview (9:16)"), width=320)
            st.download_button(
                label=t("📩 下載 IG 限動圖卡 (PNG)", "📩 Download IG Story Card (PNG)"), data=ig_buf,
                file_name=f"{player_name.replace(' ', '_')}_IG_story.png",
                mime="image/png", key="solo_download_ig",
            )

    with tab_pk:
        st.subheader(t("🥊 雙球員數據 PK / 對比模式", "🥊 Head-to-Head Player Comparison"))
        st.caption(t(
            "輸入兩位球員姓名並分別載入，即可並排比較數據面板、熱區圖與落點圖，並可生成雙人對決 PNG 圖卡。",
            "Enter and load two players to compare their stat panels, hot zones, and spray charts "
            "side by side, and generate a head-to-head PNG card.",
        ))

        col_a, col_b = st.columns(2)
        with col_a:
            st.markdown(t("#### 🔵 球員 A", "#### 🔵 Player A"))
            a_fn = st.text_input(t("名 (First Name)", "First Name"), value="Shohei", key="pk_a_fn")
            a_ln = st.text_input(t("姓 (Last Name)", "Last Name"), value="Ohtani", key="pk_a_ln")
            a_year = st.selectbox(t("賽季", "Season"), get_selectable_years(), index=0, key="pk_a_year")
            if st.button(t("📥 載入球員 A", "📥 Load Player A"), key="pk_a_load"):
                # [Fix 3] 球員 A 的抓取獨立包在自己的 try-except 中，
                # 任何未預期的例外都只影響這一側，不會波及球員 B。
                try:
                    with st.spinner(t(f"正在抓取 {a_fn} {a_ln} 的資料...", f"Fetching data for {a_fn} {a_ln}...")):
                    ...
                    st.error(t(f"⚠️ 載入球員 A（{a_fn} {a_ln}）時發生未預期錯誤：{e}",
                               f"⚠️ Unexpected error loading Player A ({a_fn} {a_ln}): {e}"))

        with col_b:
            st.markdown(t("#### 🔴 球員 B", "#### 🔴 Player B"))
            b_fn = st.text_input(t("名 (First Name)", "First Name"), value="Aaron", key="pk_b_fn")
            b_ln = st.text_input(t("姓 (Last Name)", "Last Name"), value="Judge", key="pk_b_ln")
            b_year = st.selectbox(t("賽季", "Season"), get_selectable_years(), index=0, key="pk_b_year")
            if st.button(t("📥 載入球員 B", "📥 Load Player B"), key="pk_b_load"):
                # [Fix 3] 球員 B 的抓取獨立包在自己的 try-except 中，
                # 任何未預期的例外都只影響這一側，不會波及球員 A。
                try:
                    with st.spinner(t(f"正在抓取 {b_fn} {b_ln} 的資料...", f"Fetching data for {b_fn} {b_ln}...")):
                    ...
                    st.error(t(f"⚠️ 載入球員 B（{b_fn} {b_ln}）時發生未預期錯誤：{e}",
                               f"⚠️ Unexpected error loading Player B ({b_fn} {b_ln}): {e}"))
        ...
        if "pk_a" in st.session_state or "pk_b" in st.session_state:
                ...
                if "pk_a" in st.session_state:
                    try:
                        ...
                        if pa_df_a is None or pa_df_a.empty:
                            st.warning(t(f"⚠️ 球員 A（{name_a}）於選定賽季無有效例行賽數據，暫無法顯示面板。",
                                         f"⚠️ Player A ({name_a}) has no valid regular-season data for the selected season; unable to show the panel."))
                        else:
                        ...
                        st.warning(t(f"⚠️ 球員 A 面板渲染時發生問題，已略過此側顯示：{e}",
                                     f"⚠️ Error rendering Player A's panel; this side has been skipped: {e}"))
                else:
                    st.info(t("👆 尚未載入球員 A 的資料。", "👆 Player A's data has not been loaded yet."))
                ...
                if "pk_b" in st.session_state:
                    try:
                        ...
                        if pa_df_b is None or pa_df_b.empty:
                            st.warning(t(f"⚠️ 球員 B（{name_b}）於選定賽季無有效例行賽數據，暫無法顯示面板。",
                                         f"⚠️ Player B ({name_b}) has no valid regular-season data for the selected season; unable to show the panel."))
                        else:
                        ...
                        st.warning(t(f"⚠️ 球員 B 面板渲染時發生問題，已略過此側顯示：{e}",
                                     f"⚠️ Error rendering Player B's panel; this side has been skipped: {e}"))
                else:
                    st.info(t("👆 尚未載入球員 B 的資料。", "👆 Player B's data has not been loaded yet."))
            ...
            if result_a is not None and result_b is not None:
                ...
                st.markdown("---")
                st.markdown(t("##### 📈 雙人滾動 OPS 對比（近 15 場）", "##### 📈 Rolling OPS Comparison (Last 15 Games)"))
                fig_pk_rolling = render_pk_rolling_ops_chart(pa_df_a, name_a, pa_df_b, name_b, window=15)
                if fig_pk_rolling is None:
                    st.warning(t("⚠️ 雙方例行賽資料不足，暫無法繪製滾動 OPS 對比圖。",
                                 "⚠️ Not enough regular-season data for both players to plot the rolling OPS comparison."))
                else:
                    st.plotly_chart(fig_pk_rolling, use_container_width=True, key="pk_rolling_ops")

                st.markdown("---")
                if st.button(t("🖼️ 生成雙人 PK 對決圖卡 (PNG)", "🖼️ Generate Head-to-Head Card (PNG)"), key="pk_export"):
                    st.caption(t(
                        "⏳ 圖表合成中，約需 5~10 秒（雙人需合成 2 張報告卡，相同參數重複產生時會直接讀取快取，秒開）...",
                        "⏳ Compositing the card, roughly 5–10 seconds (two report cards need to be built; "
                        "instant if the same parameters were generated before, via cache)...",
                    ))
                    with st.spinner(t("正在合成雙人 PK 對決圖卡...", "Compositing head-to-head card...")):
                    ...
                    st.image(pk_buf, caption=t("PK 對決圖卡預覽", "Head-to-Head Card Preview"), width=700)
                    st.download_button(
                        label=t("📩 下載 PK 對決圖卡 (PNG)", "📩 Download Head-to-Head Card (PNG)"), data=pk_buf,
                ...
                st.info(t(
                    "👆 只有一位球員成功載入完整資料；請載入另一位球員，即可解鎖雙線滾動 OPS 對比圖與 PK 對決圖卡。",
                    "👆 Only one player has loaded complete data. Load the other player to unlock the "
                    "rolling OPS comparison chart and the head-to-head card.",
                ))
        else:
            st.info(t("👆 請先分別載入球員 A 與球員 B 的數據。", "👆 Load data for Player A and Player B to get started."))
